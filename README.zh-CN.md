...
- **P0.5：** 第一个 Windows 便携版内部 beta 正在发布。签名安装包、自动更新、运维分发还没有完成。
...
这还是内部 beta。签名安装包分发、配额管理、直接生图/生视频、自动更新、最终成片评分等能力还没有完全完成。

## Release 状态

第一个 Windows 内部 beta 以 **Windows x64 便携版** 形式发布：

[v0.1.0-internal-beta](https://github.com/prokids-official/VideoAPP/releases/tag/v0.1.0-internal-beta)

当前建议：

- 普通同事可以下载便携版 zip，解压后运行 `FableGlitch Studio.exe`。
- 开发和测试人员可以用 `npm run dev` 本地运行。
- 维护者可以用 `npm run dist:portable` 在本机生成 Windows 便携版目录。
- 签名 NSIS 安装包仍然在计划中。当前 Windows 打包机需要先解决 symlink/签名工具权限，`npm run dist` 才能完整跑完安装包步骤。

当前第一个版本：**v0.1.0-internal-beta**。
...
1. 从 [GitHub Releases](https://github.com/prokids-official/VideoAPP/releases) 下载最新 Windows 便携版 zip。
2. 解压到本地文件夹。
3. 运行 `FableGlitch Studio.exe`。
4. 使用公司邮箱账号登录。
5. 从首页进入对应工作区：
   - **公司项目：** 查看剧集和共享资产。
...
因为第一个 beta 还没有代码签名，Windows 可能会弹出安全提醒。确认这是内部可信构建后再选择保留/运行。
...
`npm run dist:portable` 会在 `release/win-unpacked/` 下生成未签名的 Windows 便携版目录。
...
| P0.5 | 分发和运维：Windows 便携版、签名安装包、自动更新、NAS/备份、发布检查清单 | 便携版 beta 发布中 |
...
1. 用生产 Vercel 后端完整冒烟测试第一个 Windows 便携版 beta。
2. 修复 Windows 安装包打包：开启 symlink/签名工具权限，或切换到无需签名工具的安装包路径。
