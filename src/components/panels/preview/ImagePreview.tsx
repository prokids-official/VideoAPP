...
export function ImagePreview({ src, alt, onCopyImage }: { src: string; alt: string; onCopyImage?: () => void }) {
  ...
  return (
    <div
      className="relative flex min-h-0 items-center justify-center overflow-hidden rounded-lg border border-border bg-surface-2 p-3"
    ...
    >
      ...
      {failed ? (
        ...
        <img
          src={src}
          alt={alt}
          className={`max-h-full max-w-full object-contain ${loaded ? 'block' : 'hidden'}`}
