...
| **P4.5** | "看成片、打分、返工" | 最终视频评审 / 评分 / 改进建议 / 版本对比（详见 §4.5） |
| **P5** | 画布 / 3D 编排（探索） | 外部画布嵌入 + 自建导演生产台（详见 §5） |
...
## 4.5 P4.5 · 成片评审 / 打分系统（NEW, 2026-05-05）

### 4.5.1 核心目标

App 不只负责把资产生产出来，还要能在内部浏览最终作品视频，并对作品做结构化评分。评分不是只给一个总分，而是把"哪里好、哪里要返工、应该回到哪个生产阶段修"说清楚。

### 4.5.2 用户流程

1. 用户把最终视频导入到项目或公司剧集的 `03_Export / 05_Deliver` 区域。
2. App 内打开成片播放器。
3. 右侧显示评分面板：
   - 总分
   - 分项评分
   ...
   - 返工建议
   - 关联到具体时间码 / 镜头 / 资产
4. 用户可以人工打分，也可以让评分 Agent 基于乐美林已有评分文档生成初评。
5. 低分项可以生成返工任务，例如"回到 SHOT 07 的视频提示词重写"或"角色声音不稳定，回到声音资产阶段"。

### 4.5.3 评分维度占位

评分细则由乐美林已有文档决定，P4.5 spec 启动时必须先读该文档。当前先保留这些大类：
...
create table work_reviews (
  ...
  asset_id uuid references assets(id), -- 最终视频 / export 资产
  reviewer_id uuid references users(id),
  reviewer_type text check (reviewer_type in ('human', 'agent')),
...
create table work_review_items (
  id uuid primary key default gen_random_uuid(),
  review_id uuid references work_reviews(id) on delete cascade,
  timecode_start_ms int,
  timecode_end_ms int,
  related_asset_id uuid references assets(id),
  severity text check (severity in ('info', 'minor', 'major', 'blocker')),
  category text,
  note text,
...
### 4.5.5 与现有评分 Agent 的关系

P1 的"剧本评分官"只评 SCRIPT；P4.5 是"成片评分官"，输入最终视频 + 全项目资产谱系。两者共用 provider / skill / usage / quota 基础设施，但评分 prompt、输入上下文和输出 schema 不同。

---

## 5. P5 · 画布 / 导演生产台 / 3D 编排（2026-05-05 更新）

### 5.0 方向修正：不要复制无限画布，要做导演生产台

LibLib / RunningHub 这类外部画布已经是当前主要生产力，因此自建画布不应该急着复制它们的节点编辑能力。无限画布的优点是直观，缺点是工程变大后会快速变乱：线太多、节点太散、难以检查缺口，也难以把"这个视频为什么这样生成"追溯清楚。

FableGlitch 自建部分更适合做成**导演生产台**，核心是结构化、可追溯、可检查：

- **分镜时间线**：按 SHOT 组织剧本、图片 prompt、视频 prompt、输出图、输出视频。
- **资产谱系**：点任意资产都能看到上游 prompt / 参考图 / 下游生成物。
- **缺口检查**：自动发现某个 SHOT 缺图片 prompt、缺视频 prompt、缺输出。
- **返工入口**：评分系统或人工发现问题后，能一键跳回对应阶段和 SHOT。
- **视图切换**：同一批数据可以用列表、时间线、表格、谱系图、小画布查看。

结论：**自建画布仍然有必要，但它不是 P1/P2 主线，也不是另一个 LibLib。** 它应在 P5 变成我们自己的项目控制面板和轻量视觉编排层；真正重型生产继续先依赖外部画布嵌入。

### 5.0.1 推荐的创作流程形态

未来创作舱不只是一条线性 wizard，也不只是一张画布，而是三种互补视图：

1. **阶段流（Stage Flow）**
   - 灵感 → 剧本 → 角色 → 场景 → 道具 → 分镜 → 图片提示词 → 视频提示词 → 画布/生产 → 入库
   - 适合新手，不迷路。
2. **镜头表（Shot Ledger）**
   - 每一行是一个 SHOT。
   - 列包括：剧本片段、角色、场景、道具、图片 prompt、分镜图、视频 prompt、分镜视频、状态、评分。
   - 适合大工程，不会像画布一样散。
3. **资产谱系图（Asset Graph）**
   - 只在需要追溯时打开。
   - 展示某个资产的上游和下游，例如 `PROMPT_VID -> SHOT_VID -> 成片片段 -> 评分问题`。
   - 不展示全项目所有线，避免复杂工程里变成线团。

外部 LibLib / RunningHub 嵌入作为"生产视图"，负责具体生成；FableGlitch 负责计划、检查、归档、评分、返工和入库。
...
| **导演生产台 · 自建** | Shot Ledger + Asset Graph + 轻量 tldraw / React Flow 视图 | — | 不复制外部画布；专注项目结构、谱系追踪、缺口定位、返工跳转 |
...
- 2026-05-04 决策更新：LibLib 是当前外部主生产力，**必须真嵌入 App**。`shell.openExternal()` 只保留为兜底按钮，不再作为主路径。
- "在浏览器打开"按钮：
  ```js
...
### 5.3 板块 2（自建导演生产台）实施要点

- **UI 必须好看**——这是乐美林明确要求。设计稿 P5 启动时另起一份 Claude Design 任务。
- 技术选型（待 P5 启动时定）：
  - **表格/时间线主视图**：React 自建，适合 Shot Ledger。
  - **React Flow**：更适合资产谱系图，不一定用无限画布。
  - **tldraw**：适合轻量草图、摆位、手绘标注，但不作为主生产数据库。
  - **Excalidraw / fabric.js**：作为备选，不优先。
- 参考开源 repo（乐美林指）：
...
### 5.3.1 公司项目资产谱系（NEW, 2026-05-05）

公司项目浏览也需要能看到资产上下游，不只看文件预览。目标：

- 点 `PROMPT_IMG`：看到它对应哪些 `SHOT_IMG`。
- 点 `SHOT_IMG`：看到它来自哪个 `PROMPT_IMG`，并能打开 prompt 正文。
- 点 `PROMPT_VID`：看到它对应哪些 `SHOT_VID`。
- 点 `SHOT_VID`：看到它来自哪个 `PROMPT_VID`，以及使用的 `SHOT_IMG` / 首帧参考。
- 点 `CHAR` / 角色三视图：看到角色设定 prompt、三视图 prompt、参考图、服装/妆造 prompt。
- 点 `SCENE` / `PROP`：同理显示场景/道具 prompt 与生成结果。

短期实现可以先从 metadata 读取关系；长期需要后端 API 返回 related assets，前端 preview modal 做"上游 / 下游 / 同 SHOT"区域。

### 5.3.2 角色 / 场景 / 道具 prompt 建模（NEW, 2026-05-05）

分镜图和分镜视频已经有 `PROMPT_IMG -> SHOT_IMG`、`PROMPT_VID -> SHOT_VID` 的链路。角色、场景、道具也需要同样保留来源 prompt。

先不急着新增公司 `asset_types`。P1.2/P1.3 可先在本地 `meta_json` 标准化：
...
等公司资产库需要长期复用时，再决定是否新增 `CHAR_PROMPT` / `SCENE_PROMPT` / `PROP_PROMPT` 类型。这个决策需要乐美林确认，因为会影响 Supabase `asset_types`、GitHub/R2 路径、入库命名和公司浏览 UI。
...
6. ~~**3D 编排 / LibLib 嵌入**——优先级和路径~~
   ✅ 已回答 (2026-04-29)，并于 2026-05-04 修正：双板块结构。"画布 · LibLib"必须用 Electron `BrowserView` 真嵌入网页画布（不是 iframe，绕过 X-Frame-Options，持久化 LibLib cookie），`shell.openExternal()` 只作为兜底；"画布 · 自建"后续用 tldraw / Excalidraw / fabric.js 做 React 内嵌画布。3D 摆位作自建板块的进阶模式（Three.js + drei），P5 内部低优先级。详见 §5
