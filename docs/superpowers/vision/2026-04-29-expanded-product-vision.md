...
### 5.1 双板块（已锁定 2026-04-29）

| 板块 | 实现方式 | 用户体验 |
|---|---|---|
| **画布 · 自建** | tldraw / Excalidraw / fabric.js 真嵌入 React | 在 App 里直接画，资产自动落到我们 R2 / GitHub |
| **画布 · LibLib（外部）** | 一个大按钮 "在 LibLib 打开" → `shell.openExternal()` 用默认浏览器跳转 | 用户在浏览器画完，导出 PNG，拖回 App 走"图片导入"流程 |

#### 为什么 LibLib 不真嵌入？

- LibLib 网页设了 `X-Frame-Options` 不让被 iframe（试 iframe 就空白）
- 即便用 Electron `BrowserView`（不受此限制）能加载，但：
  - 用户要单独登 LibLib 账号
  - LibLib 改版我们这边随时炸
  - 法律 / 隐私问题（嵌入第三方付费服务）
- → 用 `shell.openExternal()` 跳浏览器最干净，对用户也最熟悉
